...
import { Row, ImagenSeleccionada } from './styled';
...
},[contador, crono, trabajoActual,ejercicioActual, trabajo.rutina.length] );
...
const anteriorTrabajo = ()=>{
  setTrabajoActual( (current)=> ( current - 1 + trabajosData.length  ) % trabajosData.length );
  setEjercicioActual(0);
  setContador(5);
  setCrono(false);
}
  ...
  return (
    <div>
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;
      <button onClick={()=>setCrono(true)} disabled={crono}>INICIAR RUTINA</button>
      {/* <button onClick={btnOnOff}>PAUSAR RUTINA</button> */}
      <button onClick={()=>setCrono(!crono)}>{(crono) ? 'PAUSAR' : 'CONTINUAR'}</button>
      &nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <Row>
        { trabajo.rutina.map( (ejercicio, indice) =>   
          <div key={indice}>
          <img  src={ ejercicio.img } onClick={ ()=>setEjercicioActual(indice) }  alt="" width={20} />
